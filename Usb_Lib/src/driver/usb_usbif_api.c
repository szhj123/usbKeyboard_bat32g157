...
uint32_t USB_GetVersion (void)
{
    ...
    return version;
} /* End of function USB_GetVersion */


/******************************************************************************
Function Name   : USB_GetEvent
Description     : Get event.
Arguments       : usb_ctrl_t *p_ctrl: control structure for USB API.
Return value    : event code.
******************************************************************************/
usb_status_t USB_GetEvent (usb_ctrl_t *p_ctrl)			// XXX USB event
{
    ...
    if (g_usb_cstd_event.write_pointer != g_usb_cstd_event.read_pointer)
    {
        *p_ctrl = g_usb_cstd_event.ctrl[g_usb_cstd_event.read_pointer];
        event = g_usb_cstd_event.code[g_usb_cstd_event.read_pointer];		// XXX event
...
******************************************************************************/
usb_err_t USB_Open (usb_ctrl_t *p_ctrl, usb_cfg_t *p_cfg) /* XXX Initializes the USB module */
{
    usb_err_t       err;
    ...
    else
    {
        ...
        #endif  /* USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE */
        usb_pstd_driver_init(p_cfg);            /* XXX USB driver initialization */
        hw_usb_pmodule_init();                      /* XXX Setting USB relation register  */
        
#ifdef __COMPATIBILITY_ST__
            hw_usb_pset_dprpu();

#else
        if (USB_ATTACH == usb_pstd_chk_vbsts())		// XXX check V-bus
        {
...
Return value    : usb_err_t error code      : USB_SUCCESS,USB_ERR_NG etc.
******************************************************************************/
usb_err_t USB_Read (usb_ctrl_t *p_ctrl, uint8_t *p_buf, uint32_t size)
{
    usb_info_t  info;
    usb_er_t    err;
    ...
    if (USB_REQUEST == p_ctrl->type)
    {
        err = usb_cstd_ctrl_receive(p_ctrl, p_buf, size);			// XXX Control Rcv
    }
    else
    {
        err = usb_cstd_data_read(p_ctrl, p_buf, size);				// XXX Data Rcv
    }
...
Return value    : usb_err_t error code      : USB_SUCCESS,USB_ERR_NG etc.
******************************************************************************/
usb_err_t USB_Write (usb_ctrl_t *p_ctrl, uint8_t *p_buf, uint32_t size)
{
    usb_info_t  info;
    usb_er_t    err;
    ...
    if (USB_REQUEST == p_ctrl->type)
    {
        err = usb_cstd_ctrl_send(p_ctrl, p_buf, size);			    // XXX Control Send
    }
    else
    {
        err = usb_cstd_data_write(p_ctrl, p_buf, size);				// XXX Data Send
    }
...
Return value    : usb_err_t error code  : USB_SUCCESS,USB_ERR_NG etc.
******************************************************************************/
usb_err_t    USB_Stop (usb_ctrl_t *p_ctrl, uint16_t type)
{
    usb_info_t  info;
    usb_er_t    err;
...
usb_err_t    USB_Resume (void)
{
    ...
    return ret_code;
} /* End of function USB_Resume */


/******************************************************************************
Function Name   : USB_VbusSetting
Description     : Contol of USB VBUS.(USB Host only)
Arguments       : uint16_t      state   : HI/LOW assertion
Return value    : usb_err_t error code  : USB_SUCCESS,USB_ERR_NG etc.
******************************************************************************/
usb_err_t   USB_VbusSetting (uint16_t state)
{
...
} /* End of function USB_GetPipeInfo */
/******************************************************************************
 End of function USB_PullUp
 ******************************************************************************/

/******************************************************************************
Function Name   : USB_GetInformation
...
Return value    : usb_err_t error code  : USB_SUCCESS,USB_ERR_NG etc.
******************************************************************************/
usb_err_t   USB_GetInformation (usb_info_t *p_info)
{
    #if ((USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI)
    uint16_t    status;
    ...
    return USB_SUCCESS;
} /* End of function USB_GetInformation */


/******************************************************************************
Function Name   : USB_PipeRead
...
Return value    : usb_err_t error code  : USB_SUCCESS,USB_ERR_NG etc.
******************************************************************************/
usb_err_t  USB_PipeRead (usb_ctrl_t *p_ctrl, uint8_t *p_buf, uint32_t size)
{
...
} /* End of function USB_PipeRead */


/******************************************************************************
Function Name   : USB_PipeWrite
...
Return value    : usb_err_t error code  : USB_SUCCESS,USB_ERR_NG etc.
******************************************************************************/
usb_err_t  USB_PipeWrite (usb_ctrl_t *p_ctrl, uint8_t *p_buf, uint32_t size)
{
    #if !defined(USB_CFG_HVND_USE) && !defined(USB_CFG_PVND_USE)
    return USB_ERR_NG;
...
} /* End of function USB_PipeWrite */


/******************************************************************************
Function Name   : USB_PipeStop
Description     : USB transfer stop process for Vendor class.
Arguments       : usb_ctrl_t    *p_ctrl : Pointer to usb_ctrl_t structure
Return value    : usb_err_t error code  : USB_SUCCESS,USB_ERR_NG etc.
******************************************************************************/
usb_err_t    USB_PipeStop (usb_ctrl_t *p_ctrl)
{
    #if !defined(USB_CFG_HVND_USE) && !defined(USB_CFG_PVND_USE)
    return USB_ERR_NG;
    #else
...
usb_err_t    USB_GetUsePipe (uint16_t *p_pipe)
{
    ...
    *p_pipe = ((uint16_t)1 << USB_PIPE0);
    ...
    return USB_SUCCESS;
} /* End of function USB_GetUsePipe */


/******************************************************************************
Function Name   : USB_GetPipeInfo
...
Return value    : usb_err_t error code  : USB_SUCCESS,USB_ERR_NG etc.
******************************************************************************/
usb_err_t    USB_GetPipeInfo (usb_ctrl_t *p_ctrl, usb_pipe_t *p_info)
{
    usb_info_t  info;
    uint16_t    pipe_type;
