...
#define ADC_CONVMODE_SEQUENTIAL
...
void ADC_Init(void)
{
    ...
    CGC->PER1 |= CGC_PER1_ADCEN_Msk;    /* enables input clock supply */
...
/***********************************************************************************************************************
* Function Name: ADC_Converse_Scan
* @brief  This function starts the AD converter and returns the conversion result in the buf
*         It is suitable for software trigger mode with polling mode.
* @param  ch - specify the ad channel
* @param  sz - set the times of ad conversion
...
//-----------------------------------------------------------------------
    ...
    ADC->ADM1 = _08_AD_CONVMODE_ONESELECT | _80_AD_OPERMODE_SCAN;
	  ADC->ADM1 |= (4 - scannum) <<5 ; // scan number config	
	
    ADC->ADTRG = _00_AD_TRIGGER_SOFTWARE;
    ADC->ADM0 |= ADCE;
		ADC->ADM2  = 2; //read ch number
