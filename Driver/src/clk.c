...
void CLK_UPLL_Setting(pll_src_t src, pll_div_t div, pll_mul_t mul)
{
	...
	if (src==PLL_SR_fIH){
			tmp += _00_CGC_UPLLSR_fIH;
		}
		else{
			tmp += _80_CGC_UPLLSR_fMX;
		}

	  if (div==PLL_DIV_2){
			tmp += _04_CGC_UPLL_DIV_2;
		}
		else if(div==PLL_DIV_4){
			tmp += _08_CGC_UPLL_DIV_4;
		}
		else{
			tmp += _00_CGC_UPLL_DIV_1;
		}

	  if (mul==PLL_MUL_16){
			tmp += _02_CGC_UPLL_MUL_16_0;
		}
		else{
			tmp += _00_CGC_UPLL_MUL_12_0;
		}
...
/***********************************************************************************************************************
* Function Name: CLK_UPLL_Start
* @brief  This function starts the UPLL. 
* @param  None
* @return None
***********************************************************************************************************************/
void CLK_UPLL_Start(void)
{
	CGC->UPLLCR |= 1<<0;       /* UPLLON = 1 */
}
/***********************************************************************************************************************
* Function Name: CLK_PLL_Start
* @brief  This function starts the system PLL. 
