...
void SPI11_MasterInit(spi_mode_t mode)
{
    ...
    SCI1->SCR11 = _0004_SCRMN_DEFAULT_VALUE | _C000_SCI_RECEPTION_TRANSMISSION | dap << 13 | ckp << 12 | _0000_SCI_INTSRE_MASK |
                  _0000_SCI_PARITY_NONE | _0000_SCI_MSB | _0000_SCI_STOP_NONE | _0007_SCI_LENGTH_8;
  //SCI1->SDR11 = 0 << 9; 	// fMCK/(1+n)x2 = fMCK/2
  //SCI1->SDR11 = 1 << 9; 	// fMCK/(1+n)x2 = fMCK/4
  //SCI1->SDR11 = 2 << 9;		// fMCK/(1+n)x2 = fMCK/6
  SCI1->SDR11 = 3 << 9;		// fMCK/(1+n)x2 = fMCK/8  
	//SCI1->SDR11 = 5 << 9;		// fMCK/(1+n)x2 = fMCK/12
	//SCI1->SDR11 = 7 << 9;		// fMCK/(1+n)x2 = fMCK/16
	//SCI1->SDR11 = 15 << 9;	// fMCK/(1+n)x2 = fMCK/32
	//SCI1->SDR11 = _CE00_SCI_BAUDRATE_DIVISOR;
    /* Set output enable */
    if ((mode == SPI_MODE_0) || (mode == SPI_MODE_1))
    {
...
MD_STATUS SPI11_MasterSend(uint8_t *const tx_buf, uint16_t tx_num)
{
    ...
    else
    {
        ...
        DMAVEC->CTRL[CTRL_DATA_SPI11].DMDAR = (uint32_t)&SCI1->SIO11;
        ...
        gp_spi11_tx_address++;
        g_spi11_tx_count--;
        SCI1->SIO11 = tx_buf[0]; /* started by writing data to SDR[7:0] */
    }
...
MD_STATUS SPI11_MasterReceive(uint8_t *const rx_buf, uint16_t rx_num)
{
    ...
    else
    {
        ...
        DMAVEC->CTRL[CTRL_DATA_SPI11].DMSAR = (uint32_t)&SCI1->SIO11;
        ...
        DMAVEC->CTRL[CTRL_DATA_SPI11 + 1].DMDAR = (uint32_t)&SCI1->SIO11;
...
MD_STATUS UART2_Init(uint32_t freq, uint32_t baud)
{
    ...
    SCI2->SMR20 = _0020_SMRMN_DEFAULT_VALUE | _0000_SCI_CLOCK_SELECT_CK00 | _0000_SCI_CLOCK_MODE_CKS |
                  _0002_SCI_MODE_UART | _0000_SCI_TRANSFER_END;
    SCI2->SCR20 = _0004_SCRMN_DEFAULT_VALUE | _8000_SCI_TRANSMISSION | _0000_SCI_TIMING_1 | _0000_SCI_INTSRE_MASK |
                  _0000_SCI_PARITY_NONE | _0080_SCI_LSB | _0010_SCI_STOP_1 | _0007_SCI_LENGTH_8;
    ...
    SCI2->SMR21 = _0020_SMRMN_DEFAULT_VALUE | _0000_SCI_CLOCK_SELECT_CK00 | _0000_SCI_CLOCK_MODE_CKS |
                  _0100_SCI_TRIGGER_RXD | _0000_SCI_EDGE_FALL | _0002_SCI_MODE_UART | _0000_SCI_TRANSFER_END;
    SCI2->SCR21 = _0004_SCRMN_DEFAULT_VALUE | _4000_SCI_RECEPTION | _0000_SCI_TIMING_1 | _0000_SCI_INTSRE_MASK |
                  _0000_SCI_PARITY_NONE | _0080_SCI_LSB | _0010_SCI_STOP_1 | _0007_SCI_LENGTH_8;
...
void SPI21_MasterInit(spi_mode_t mode)
{
    ...
    //SPS0 = (0 << SCI2_SPS0_PRS00_Pos) | (3 << SCI0_SPS0_PRS00_Pos);
    SCI2->SIR21 = _0004_SCI_SIRMN_FECTMN | _0002_SCI_SIRMN_PECTMN | _0001_SCI_SIRMN_OVCTMN;
    SCI2->SMR21 = _0020_SMRMN_DEFAULT_VALUE | _8000_SCI_CLOCK_SELECT_CK01 | _0000_SCI_CLOCK_MODE_CKS |
                  _0000_SCI_TRIGGER_SOFTWARE | _0000_SCI_MODE_SPI | _0000_SCI_TRANSFER_END;
    SCI2->SCR21 = _0004_SCRMN_DEFAULT_VALUE | _C000_SCI_RECEPTION_TRANSMISSION | dap << 13 | ckp << 12 | _0000_SCI_INTSRE_MASK |
                  _0000_SCI_PARITY_NONE | _0000_SCI_MSB | _0000_SCI_STOP_NONE | _0007_SCI_LENGTH_8;
    SCI2->SDR21 = _CE00_SCI_BAUDRATE_DIVISOR;
    /* Set output enable */
    if ((mode == SPI_MODE_0) || (mode == SPI_MODE_1))
...
MD_STATUS SPI21_MasterSendReceive(uint8_t *const tx_buf, uint16_t tx_num, uint8_t *const rx_buf)
{
    ...
    else
    {
        ...
        DMAVEC->CTRL[CTRL_DATA_SPI21].DMSAR = (uint32_t)&SCI2->SIO21;
        ...
        DMAVEC->CTRL[CTRL_DATA_SPI21 + 1].DMDAR = (uint32_t)&SCI2->SIO21;
        ...
        SPI21_Start();
        SCI2->SIO21 = *gp_spi21_tx_address; /* started by writing data to SDR[7:0] */
        gp_spi21_tx_address++;
        g_spi21_tx_count--;
    }
...
MD_STATUS SPI21_MasterSend(uint8_t *const tx_buf, uint16_t tx_num)
{
    ...
    else
    {
        ...
        DMAVEC->CTRL[CTRL_DATA_SPI21].DMDAR = (uint32_t)&SCI2->SIO21;
...
MD_STATUS SPI21_SlaveSend(uint8_t *const tx_buf, uint16_t tx_num)
{
    ...
    else
    {
        ...
        gp_spi21_tx_address++;
        g_spi21_tx_count--;
        SCI2->SIO21 = tx_buf[0]; /* started by writing data to SDR[7:0] */
    }
...
void IIC21_Init(void)
{
    ...
    SCI2->SMR21 = _0020_SMRMN_DEFAULT_VALUE | _8000_SCI_CLOCK_SELECT_CK01 | _0000_SCI_CLOCK_MODE_CKS |
                  _0000_SCI_TRIGGER_SOFTWARE | _0000_SCI_EDGE_FALL | _0004_SCI_MODE_IIC | _0000_SCI_TRANSFER_END;
    SCI2->SCR21 = _0004_SCRMN_DEFAULT_VALUE | _0000_SCI_TIMING_1 | _0000_SCI_INTSRE_MASK | _0000_SCI_PARITY_NONE |
                  _0000_SCI_MSB | _0010_SCI_STOP_1 | _0007_SCI_LENGTH_8;
    SCI2->SDR21 = _CE00_SCI_BAUDRATE_DIVISOR;
